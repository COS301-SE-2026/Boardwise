...
python_user_data = python_image.image_uri.apply(
    lambda image_uri: (
        python_setup_script.replace("__IMAGE_URI__", image_uri)
...
spring_user_data = pulumi.Output.all(
    image_uri=spring_image.image_uri, python_ip=python_instance.private_ip
).apply(
    lambda args: (
        spring_setup_script.replace("__IMAGE_URI__", args["image_uri"])
        .replace(
            "__SMTP_PASSWORD__",
            settings.SMTP_PASSWORD if settings.SMTP_PASSWORD is not None else "",
        )
        .replace(
            "__SMTP_USERNAME__",
            settings.SMTP_USERNAME if settings.SMTP_USERNAME is not None else "",
        )
        .replace(
            "__SMTP_HOST__",
            settings.SMTP_HOST if settings.SMTP_HOST is not None else "",
        )
