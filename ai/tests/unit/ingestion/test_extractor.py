...
def _setup_mock_pdf(mock_pymupdf_open, document_length: int = 1):
    """
    ...
    mock_pymupdf_open.return_value.__enter__.return_value = mock_document
    return mock_document


@patch("app.ingestion.extractor.pymupdf4llm.to_markdown")
@patch("app.ingestion.extractor.pymupdf.open")
def test_extract_text_successful_parsing_returns_markdown(
    mock_pymupdf_open, mock_to_markdown, safe_pdf_bytes
):
    # Arrange
    expected_md = "# Header\n\nThis is extracted markdown text."
    mock_document = _setup_mock_pdf(mock_pymupdf_open, 5)
    ...
    mock_pymupdf_open.assert_called_once_with(stream=safe_pdf_bytes, filetype="pdf")
