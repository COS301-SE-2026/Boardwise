...
*   **Containerisation**: The two backend deployables - the Spring Boot Transactional Backend (running on JDK 21 Eclipse Temurin) and the FastAPI AI Backend - each have their own Dockerfile pinning an explicit base image tag (e.g. `eclipse-temurin:21-jre-alpine` for the Transactional Backend, `python:3.12-slim` for the AI Backend), exposed port, and entry point. The Vue.js/Nuxt frontend is built as static assets (HTML/CSS/JS) rather than containerised. A root `docker-compose.yml` wires the two backend containers together for local development.
*   **Resource-Constrained Configuration**: Given CON2's shared RAM budget, the Transactional Backend's Docker image is built with an explicit JVM heap cap (e.g. `-Xmx256m`) to reduce its memory footprint and the risk of out-of-memory failures when co-located with the AI Backend on the same Fargate task allocation.
*   **IaC**: AWS Fargate/ECS task and service definitions declare each container's image, CPU/memory allocation, exposed port, environment variables, and health-check path, fronted by an Application Load Balancer that forwards `/api/**` traffic to the appropriate backend container; CloudFront serves the static frontend bundle from its origin S3 bucket. This allows a fresh environment to be reproduced without manual console configuration.
...
*   **Mechanism**: Both backend containers read secrets from injected environment variables at runtime, supplied to their Fargate task definitions.
*   **Production Secret Store**: GitHub Actions Secrets (for CI/CD-time injection) plus AWS's own secret-injection mechanism for ECS task definitions (Secrets Manager or SSM Parameter Store), with a documented `.env.example` in each backend's root folder listing every required variable name without values. The JWT signing secret is provisioned once and shared identically between the Transactional Backend and the AI Backend, so the AI Backend can validate tokens independently without a live call back to the Transactional Backend.
...
*   **Procedure**: If a new Fargate task revision fails its post-deploy health check, the ECS service halts the rollout and continues routing traffic to the previous stable task revision; a failed pipeline run additionally triggers a notification so the team can either fix-forward or manually re-trigger deployment of the last known-good tag.
...
*   **AWS (`eu-north-1`)**: CloudFront (edge caching, HTTPS/SSL termination) sourced from an S3 "Static Content" bucket serving the built frontend; an Application Load Balancer forwarding API traffic into a Serverless Compute Engine (**AWS Fargate/ECS**) running two Docker containers on a shared JVM/host allocation - the Spring Boot **Transactional Backend** and the Python/FastAPI **AI Backend**.
...
*   **Communication Paths**: HTTPS for all client-to-edge and edge-to-origin traffic; WSS (WebSocket over TLS) for Vault real-time broadcasts, terminated at the same Load Balancer as REST traffic; the MongoDB wire protocol (TLS-secured `mongodb+srv://`) between the Transactional Backend and Atlas; HTTPS to the R2 S3-compatible API and to the two external APIs.
*   **Environment Scope**: this diagram represents the **Production** topology; the Staging topology mirrors it on a separate Fargate service definition and a separate Atlas database name (4.1).
...
5.  **Deployment Stage**: The relevant ECS service is updated to the newly pushed task/image revision, followed by an automated post-deploy health-check request to each backend container's `/health` endpoint before traffic is shifted over; the CloudFront distribution is invalidated to pick up the newly uploaded static bundle.
6.  **Failure Path & Notifications**: A failed test, or build stage blocks the merge/deploy; a failed post-deploy health check keeps the previous stable ECS task revision serving production (see 4.4).
